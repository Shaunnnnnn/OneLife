...
if(isset($_POST['fullname'])){
  ...
  //session input variable
  $session_input =$_POST["sessions"];
  $dor = $_POST["dor"];
  $gender = $_POST["gender"];
  ...
  //find reminder countdown
  date_default_timezone_set("Africa/Harare");
  $date_of_registration = date_format(new DateTime($dor),"Y-m-d");
  if ($plan == "Sessions"){
    $expiry_date = date("Y-m-d", strtotime($date_of_registration) + (60 * 60 * 24 * 30));
    }else {
      $expiry_date = date("Y-m-d", strtotime($date_of_registration) + (60 * 60 * 24 * ($plan)));
    }
  ...
  //if ($session_input != 0){
  if( strtotime($current_date) >= strtotime($dor) ){
    if ($plan == "Sessions"){
...
$qry = "INSERT INTO members(fullname,dor,gender,services,amount,p_year,paid_date,plan,contact,sessions,expiry_date,status) values ('$fullname','$dor','$gender','$services','$totalamount','$p_year','$paid_date','$plan','$contact','$session_input','$expiry_date','$status')";
